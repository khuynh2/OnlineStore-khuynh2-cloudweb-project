...
export async function cart_page(){
    ...
    const checkoutButtom = document.getElementById(`button-checkout`);
    checkoutButtom.addEventListener('click',async  () => {
        const label = Util.disableButton(checkoutButtom);
        try {
        ...
        Util.enableButton(checkoutButtom,label);
    })
}