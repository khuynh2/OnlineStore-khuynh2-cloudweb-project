...
export async function profile_page() {
    ...
    html += `
      <table>
        <tr>
          ...
          <td>
             <button id ="profile-phot-update-button" class ="btn btn-outline-danger">Update Photo</button>
          </td>
        </tr>
    ...
    const updateProfilePhotoButton = document.getElementById('profile-phot-update-button');
    updateProfilePhotoButton.addEventListener('click', async () => {
        if (!photoFile) {
            Util.info('No Photo Selcted', 'Choose a profile photo');
            return;
        }
...
export async function getAccountInfo(user) {
    try {
        ...
        if (Constants.DEV) console.log(e)
        Util.info(`Failed to retrived account info for ${user.email}`, JSON.stringify(e));
        accountInfo = null;
        return;
    }
    Element.menuProfile.innerHTML = `
